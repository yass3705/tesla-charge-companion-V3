...
function normalizeStation(st,defaults={}){
 let isTesla=(st.source||defaults.source)==='teslaSupercharger'||(st.operator||defaults.operator)==='Tesla';
 if(isTesla&&defaults.id){
   // Official Tesla data wins for fields refreshed by GitHub Actions.
   return {...st,...defaults,
     operator:'Tesla',
     temporarilyUnavailable:st.temporarilyUnavailable??defaults.temporarilyUnavailable??false,
     pricing:normalizePricingCurrency(defaults.pricing||st.pricing),
     stalls:defaults.stalls||st.stalls||0,
     customUserNote:st.customUserNote||''
   };
 }
 return {...defaults,...st,
   operator:st.operator||defaults.operator||'',
   stalls:st.stalls||defaults.stalls||0,
   pricing:normalizePricingCurrency(st.pricing||defaults.pricing)
 };
}
function saveLocal(){localStorage.setItem('tccStationsV30',JSON.stringify(stations))}
...
function showTeslaSyncInfo(){
 $('teslaSyncInfo').innerHTML=`La mise à jour est prévue chaque semaine et peut aussi être lancée manuellement depuis GitHub : <b>Actions → Mise à jour Tesla → Run workflow</b>. Après le commit automatique, GitHub Pages republie la nouvelle base.`;
}
